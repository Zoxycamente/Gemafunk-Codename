...
float size = 10000.; // Pixelated resolution x-component
float threshold = .01; // Threshold for dithering (0.0045 found to be optimal)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    if (needs_dither(sampleColor)) {
        ...
        //fragColor = vec4(closest_gb(texture(iChannel0, uv).xyz), texture(iChannel0, fragCoord / iResolution.xy).a);
