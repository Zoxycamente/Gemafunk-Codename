...
function onNoteCreation(event) {
	...
	if (event.note.isSustainNote) {
		note.loadGraphic(Paths.image('game/notes/arrowEnds'), true, 7, 6);
		note.animation.add("hold", [event.strumID]);
		note.animation.add("holdend", [4 + event.strumID]);
	} else {
		note.loadGraphic(Paths.image('game/notes/arrows-pixels'), true, 17, 17);
...
function onPostNoteCreation(event) {
	...
	if (pixelSplashes)
		splashes.splash = "pixel-default2";
}
